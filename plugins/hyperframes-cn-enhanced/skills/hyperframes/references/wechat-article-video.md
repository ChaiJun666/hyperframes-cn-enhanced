...
If this repository includes `templates/wechat-video/`, use that template as the execution scaffold. This reference provides scene design rules and pitfalls; the template owns project creation, artifact paths, edge-tts generation, timeline writing, and validation commands.
...
1. **Images**: all Markdown image references such as `![alt](...)` — article screenshots/illustrations
...
|:---|:---|:---|
| **cinema-title** | Generated editorial background + bottom-aligned hero text + pill tags | Opening title, closing message |
...
- **Text density**: follow tts-workflow.md guidelines. Keep narration concise, generate audio, then use measured audio duration to set scene timing.

Generate audio with edge-tts by default (see `external-tts.md` for full details). Do not require a TTS API key unless the user explicitly asks for another provider.
...
Do not hardcode a watermark or brand bar. Only include one when the user asks for it or `VIDEO_BRAND_TEXT` is set:
```html
<div class="brand-bar"><span>YOUR_BRAND</span></div>
```
...
- `external-tts.md` — edge-tts voice generation and timing details
- `news-flash-images.md` — generated background image planning and local asset handling

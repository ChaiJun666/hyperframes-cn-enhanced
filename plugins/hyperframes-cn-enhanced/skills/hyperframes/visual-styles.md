...
**Mood:** Urgent, high-impact | **Best for:** financial news, market updates, data-heavy short videos, WeChat article summaries

- Use generated editorial backgrounds or article images, then add dark gradient overlays around `0.40-0.60` alpha so images stay visible.
- Palette: alert red (`#e11d48`), signal yellow (`#facc15`), ink black (`#050505`), off-white (`#f8fafc`), data cyan (`#22d3ee`).
- Typography should feel like social news packaging: heavy condensed headlines, large numeric callouts, short labels, and high contrast.
- Layout uses three readable zones: headline, key data/quote, and takeaway. Favor vertical stacking in portrait video.
- Motion is fast and editorial: blur crossfades, push slides, ticker-like reveals, numeric punches, and short holds.
- Use [references/news-flash-images.md](references/news-flash-images.md) for generated image prompts, local asset handling, overlay strength, and overflow checks.

```
News Flash: real-photo background + visible dark overlay. Alert red/yellow/cyan accents.
Heavy Chinese headlines, large numeric callouts, short data rows. Fast editorial motion.
Best for urgent data-heavy short videos and WeChat article summaries.
```
...
These 9 styles are examples — not constraints. Create your own by:

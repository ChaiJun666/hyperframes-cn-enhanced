---
name: hyperframes
description: Create video compositions, animations, title cards, overlays, captions, voiceovers, audio-reactive visuals, and scene transitions in HyperFrames HTML. Use when asked to build any HTML-based video content, add captions or subtitles synced to audio, generate text-to-speech narration, integrate TTS voiceover (edge-tts, OpenAI, etc.) with per-scene audio segmentation and sync, create audio-reactive animation (beat sync, glow, pulse driven by music), add animated text highlighting (marker sweeps, hand-drawn circles, burst lines, scribble, sketchout), or add transitions between scenes (crossfades, wipes, reveals, shader transitions). Covers composition authoring, timing, media, and the full video production workflow. Also triggers on WeChat article URLs (mp.weixin.qq.com) to auto-parse and produce video. For CLI commands (init, lint, preview, render, transcribe, tts) see the hyperframes-cli skill.
---
...
### Step 4: Archive the video deliverable

For every new video or update to an existing video, create a dedicated product folder under `artifacts/videos/<YYYY-MM-DD>-<video-slug>/` unless the user provides another destination. Keep the rendered video and all materials needed to reproduce it together in that folder:

- HyperFrames project files (`index.html`, `design.md`, `hyperframes.json`, `package.json`, scripts)
- generated or sourced images, audio, narration timelines, transcripts, and other assets
- final renders under `renders/`
- short notes when validation or render commands needed special flags

Do not overwrite a previous video update in place. If the same topic is revised later, create a new dated product folder or a clearly named revision folder so each delivered version remains inspectable.
...
When using `::after` pseudo-elements as gradient overlays on background images, keep alpha values between **0.40 and 0.60**. Values above 0.80 completely wash out the background image, making it invisible. This is especially important for the News Flash 信息流快消 style, where generated editorial backgrounds should remain visible behind the text.
...
- **[visual-styles.md](visual-styles.md)** — 9 named visual styles with hex palettes, GSAP easing signatures, and shader pairings. Includes News Flash (信息流快消) for urgent, data-heavy financial/news content with generated editorial backgrounds. Read when user names a style or when generating design.md.
- **[news-flash-images.md](references/news-flash-images.md)** — Codex image generation workflow for News Flash style: scene-specific prompts, local image asset handling, overlay darkness guide, three-segment layout sizing, and inspect overflow handling.

...
def load_dotenv_values():
    ...
    for key in ("EDGE_TTS_VOICE", "EDGE_TTS_RATE", "EDGE_TTS_VOLUME", "EDGE_TTS_PITCH"):
...
def build_timeline(audio_results):
    current = 0.0
    timeline = []
    for item in audio_results:
        duration = max(item["audio_dur"] + 2.0, 6.0)
        timeline.append({
...
def render_templates(run_dir, scene_plan, timeline):
    ...
    for idx in range(len(timeline) - 1):
        ...
        transition_html.append(
            f'<div class="clip transition fade-transition" data-start="{start}" '
